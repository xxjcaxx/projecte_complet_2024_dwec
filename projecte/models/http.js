...
const signup = async (email, password) => {
  try {
    let response = await fetch(`${urlBase}auth/v1/signup`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        apikey,
...
const expirationDate = (expires_in) =>
  Math.floor(Date.now() / 1000) + expires_in;

const login = async (email, password) => {
  const status = { success: false };
  try {
    let response = await fetch(`${urlBase}auth/v1/token?grant_type=password`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        apikey,
...
const getProfile = () => {
  ...
  return from(getSupabase("profiles", "*", `&id=eq.${uid}`)).pipe(
...
const updateProfile = async (profile) => {
  ...
  const avatarResponse = await fileRequest(
